...
** Variables
   ...
   | 4 |
** Variable Expansions
   This is bash/zsh only. They offer some variable manipulations using
   shell only, not having to create another process =sed,awk,perl=.
   ...
   Take a read on
   https://tldp.org/LDP/abs/html/string-manipulation.html#SUBSTRREPL00
   for more details.
   ...
   This will produce =--env var1 --env var2=.  Super useful to be
...
** change loops for "mapping" functions
*** find
    ...
    #+begin_src bash
      ...
      find . -type f -name "*docx" -exec pandoc "{}" -t plain \; | wc
...
** adding =bash= to a script to debug
   ...
   #+begin_src bash
     ...
     docker run -it -v $PWD:/tmp/file.sh mycomplex-image /tmp/file.sh
...
* Footnotes
[fn:1] https://gist.github.com/samth/3083053df
