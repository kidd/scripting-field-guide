...
*** grep -Fvf
    ...
    #+end_src
*** jq
    Jq is probably more powerful than you'd expect. It's not just to
    fetch a value deep into a json dictionary.
...
EOF
   ...
   #+begin_src bash
     deploy () {
       ...
       ls -tdr ~/my-project/artifacts/deploy-be-* | head -n -5 | xargs rm -rf
