...
** adding =bash= to a script to debug
   ...
   Let's compare some usual uses of docker and how we can debug our scripts there

   #+begin_src bash
     # leaves you at a console to fiddle if everything is in place
     docker run -it mycomplex-built-image bash
     # Runs /tmp/file.sh from the host inside
     docker run -it -v $PWD:/tmp/file.sh ubuntu /tmp/file.sh
     echo 'bash' >>$PWD/file.sh
     # edit the script adding a call to bash
     docker run -it -v $PWD:/tmp/file.sh ubuntu /tmp/file.sh
