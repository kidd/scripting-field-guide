...
** Array slices
   ...
   This is a [[https://www.gnu.org/software/bash/manual/html_node/Shell-Parameter-Expansion.html][parameter expansion]], same as =${a/foo/bar}= and similar
   replacements. As such, The =$@= and =$*= are treated a bit
   differently.

   Look at =${parameter:offset:length}= in the parameter expansion
   bash manual and be amazed by the specialcasing :).

   This concrete case is when the parameter is an indexed array name
   subscripted by '@' or '*'.
...
** Conditional Expansion
   ...
   #+begin_src bash
   local  foo=${var:-${default:-}}
   local  foo=${var:+&key=var}
   #+end_src
...
** Modify behavior by linking files
   Imagine you want to remove the =.bash_history= file and not record
   history of commands.  You don't can't avoid bash to write to that
   file, but what you can do is to link =.bash_history= to
   =/dev/null=. That way, bash will keep appending things to the
   expected output file, without noticing that it is really writing to
   =/dev/null=. The trick is to use: =ln -sf /dev/null .bash_history=.
...
** Scripts as filters
   ...
   Another way to create filters is to use `"${@:-cat}"`.

   #+begin_src bash
     o2e     () { 1>&2 "${@:-cat}"; } # out to err
     e2o     () { 2>&1 "${@:-cat}"; } # err to out
   #+end_src
