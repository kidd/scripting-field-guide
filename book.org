...
** eval?
   ...
   In the following snippet, we are in release script. Some step
   ...
   #+begin_src bash
    test_kong "$PACKAGE_PATH" $(awk '/^FROM /{print $2; exit}' $LOCAL_PATH/$(get_dockerfile_for $DISTRO) | sed -e "s/\$VERSION/$VERSION/")
   #+end_src
...
** Interpolation
   ...
   | book.org   |
   | readme.org |
** use return values for conditionals
...
** Do work on loop conditions
   Although not commonly seen, =while= conditions are just plain
   commands, so you can put other stuff than =[]= tests there.
...
** One Branch Conditionals
   The usual conditionals one sees everywher is =if=.
   #+begin_src bash
...
* Docker                                                           :noexport:
** Use volumes as a conveyor belt
** mount .bashrc to have a shared history
** monkeypatch docker run command to inject .bashrc shared file
** Randomize ports. or use docker port
* patterns                                                         :noexport:
